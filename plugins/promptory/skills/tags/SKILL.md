...
prompts as a numbered list. For every prompt, format `1. Prompt title — Prompt
description` when one exists; show only `1. Prompt title` when there is no
...
actions, request explicit confirmation. Never expose tokens or request `user_id`.
